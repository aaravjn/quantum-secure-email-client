...
from crypto.crystal.polynomials import *
from crypto.crystal.modules import *
from crypto.crystal.ntt_helper import NTTHelperKyber

try:
    from crypto.crystal.aes256_ctr_drbg import AES256_CTR_DRBG
...
class Kyber:
    ...
    def _xof(bytes32, a, b, length):
        ...
        if len(input_bytes) != 34:
            raise ValueError(
                f"Input bytes should be one 32 byte array and 2 single bytes."
            )
    ...
    def _prf(s, b, length):
        ...
        if len(input_bytes) != 33:
            raise ValueError(
                f"Input bytes should be one 32 byte array and one single byte."
            )
