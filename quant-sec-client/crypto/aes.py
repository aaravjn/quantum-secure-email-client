...
def encrypt(plain_text, passkey):
    ...
    private_key = hashlib.scrypt(passkey.encode(), salt=salt, n=2**14, r=8, p=1, dklen=32)
    padded_text = pad(plain_text)
    cipher_config = AES.new(private_key, AES.MODE_ECB)

    return {
        'cipher_text': str(base64.b64encode(cipher_config.encrypt(bytes(padded_text, "UTF-8"))), encoding='utf-8'),
